...
//获取部门员工
function get_role_admin($where){
   $sql = 'SELECT user_id,user_name FROM '.$GLOBALS['ecs']->table('admin_user')." WHERE status=1 AND stats>0 $where ORDER BY convert(user_name using gbk ) ASC"; 
   return $GLOBALS['db']->getAll($sql);
}
